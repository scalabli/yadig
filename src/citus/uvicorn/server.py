...
if TYPE_CHECKING:
    from .protocols.http.h11_impl import H11Protocol
    from .protocols.http.httptools_impl import HttpToolsProtocol
    from .protocols.websockets.websockets_impl import WebSocketProtocol
    from .protocols.websockets.wsproto_impl import WSProtocol
...
class Server:
    ...
    async def serve(self, sockets: Optional[List[socket.socket]] = None) -> None:
       ...
       # color_message = quo.echo(f"Started server process {%d}", fg="cyan")
        color_message = "🚀 Started server process [" + flair("%d", fg="cyan") + "]"
        ...
        color_message = "Finished server process [" + flair("%d", fg="cyan") + "]"
        logger.info(message, process_id, extra={"color_message": color_message})
    ...
    def _log_started_message(self, listeners: List[socket.SocketType]) -> None:
        ...
        if config.fd is not None:
            sock = listeners[0]
            logger.info(
                "Citus is running on socket %s (Press CTRL+C to quit)",
                sock.getsockname(),
            )

        elif config.uds is not None:
            logger.info(
                "Citus is running on unix socket %s (Press CTRL+C to quit)", config.uds
            )
            ...
            message = f"Citus is running on {addr_format} (Press CTRL+C to quit)"
            color_message = (
                "Citus is running on "
